...
- **TICKET 4 COMPLETE**: Unified email conversion service with `PDF_CONVERTER_ENGINE` feature flag. When flag=cloudconvert, replaces Puppeteer email body PDF creation with CloudConvert processing. Supports unified ConvertInput array for email body + attachments, graceful fallback to Puppeteer, and maintains full backward compatibility. Enhanced analytics include conversion engine tracking.
- **TICKET 6 COMPLETE**: Enhanced error handling, retries, and user-visible states for CloudConvert operations. Implements 3x exponential backoff retry policy, comprehensive error mapping (401/403→configuration_error, 422→skipped_password_protected, 415→skipped_unsupported, timeout→retried), and user-friendly status badges. Features Sentry integration with CloudConvert job tracking, ensures failures don't block original storage, and provides actionable feedback through status messages like "Conversion skipped (password required)" and "Conversion failed (temporary)".
- **TICKET 7 COMPLETE**: Comprehensive observability & metrics system providing visibility into conversion performance without overhauling monitoring infrastructure. Emits structured metrics for pdf.convert.duration_ms{engine,type}, success_total, error_total{reason}, and retry_total with Sentry breadcrumb integration. Features per-email conversion summaries, performance statistics (p50/p95 duration, error rates), automatic error classification, and REST API endpoints (/api/metrics/performance, /api/metrics/email-summaries) for dashboard consumption. Includes memory-efficient metrics collection with configurable cleanup and real-time logging for operational visibility.
- **EMAIL ENGINE DECISION SYSTEM COMPLETE**: Database-backed feature flags for email conversion engines with proper precedence handling (ENV > DB flags > defaults). Implements `EMAIL_BODY_PDF_USE_CLOUDCONVERT` and `EMAIL_ATTACHMENT_CONVERT_TO_PDF` flags with per-user rollout capabilities, comprehensive observability, and instant rollback mechanisms. Engine decision service provides drop-in wrapper with decision reasoning for operational visibility.
