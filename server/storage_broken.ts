...
export interface IStorage {
  ...
  searchDocuments(userId: string, searchTerm: string, limit?: number): Promise<Array<Document & { relevanceScore?: number, matchType?: string }>>;
...
export class DatabaseStorage implements IStorage {
  ...
  // Advanced search with ranking for search-as-you-type functionality
  async searchDocuments(userId: string, searchTerm: string, limit: number = 10): Promise<Array<Document & { relevanceScore?: number, matchType?: string }>> {
    ...
    // Use a simpler approach for now - can be enhanced later with full-text search
    const baseQuery = this.db
      .select({
        ...
        originalMimeType: documents.originalMimeType,
        conversionJobId: documents.conversionJobId,
        conversionMetadata: documents.conversionMetadata,
        conversionEngine: documents.conversionEngine,
        conversionInputSha256: documents.conversionInputSha256,
        conversionReason: documents.conversionReason,
        derivedFromDocumentId: documents.derivedFromDocumentId,
      ...
      .where(
        and(
          ...
          or(
            ...
            sql`EXISTS (SELECT 1 FROM unnest(COALESCE(${documents.tags}, ARRAY[]::text[])) AS tag WHERE LOWER(tag) LIKE ${`%${lowerSearchTerm}%`})`
          )
        )
      )
      .orderBy(desc(documents.uploadedAt))
    ...
    // Add relevance scoring in JavaScript for simplicity
    return results.map(doc => {
      let relevanceScore = 0;
      let matchType = 'other';

      const nameMatch = doc.name?.toLowerCase().includes(lowerSearchTerm);
      const fileNameMatch = doc.fileName?.toLowerCase().includes(lowerSearchTerm);
      const extractedTextMatch = doc.extractedText?.toLowerCase().includes(lowerSearchTerm);
      const summaryMatch = doc.summary?.toLowerCase().includes(lowerSearchTerm);
      const tagMatch = doc.tags?.some(tag => tag.toLowerCase().includes(lowerSearchTerm));
      const emailSubjectMatch = (doc.emailContext as any)?.subject?.toLowerCase().includes(lowerSearchTerm);
      const emailFromMatch = (doc.emailContext as any)?.from?.toLowerCase().includes(lowerSearchTerm);

      if (doc.name?.toLowerCase() === lowerSearchTerm) {
        relevanceScore = 100;
        matchType = 'title';
      } else if (doc.name?.toLowerCase().startsWith(lowerSearchTerm)) {
        relevanceScore = 90;
        matchType = 'title';
      } else if (nameMatch) {
        relevanceScore = 60;
        ...
        relevanceScore = 40;
        matchType = 'summary';
      } else if (emailSubjectMatch || emailFromMatch) {
        relevanceScore = 30;
        matchType = 'email';
