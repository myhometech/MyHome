...
export function InsightCard({ insight, onStatusUpdate, onDocumentClick, onDelete }: InsightCardProps) {
  ...
  const handleCardClick = (e: React.MouseEvent) => {
    ...
    const docId = insight.documentId;
    if (!docId || docId <= 0) {
      return;
    }
  ...
  const hasValidDocument = insight.documentId && insight.documentId > 0;

  return (
    <Card 
      className={`group relative overflow-hidden rounded-xl transition-all duration-300 ${hasValidDocument ? 'cursor-pointer' : 'cursor-default'} transform hover:scale-[1.02] hover:shadow-xl ${config.borderColor} border-2 ${
