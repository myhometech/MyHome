...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Document Health Score API (must come before parameterized routes)
  app.get('/api/documents/health-score', requireAuth, async (req: any, res) => {
    try {
      const userId = getUserId(req);
      
      // Get user's documents and stats
      const documents = await storage.getDocuments(userId);
      const categories = await storage.getCategories(userId);
      
      // Calculate health score components
      const totalDocuments = documents.length;
      const documentsWithCategories = documents.filter(d => d.categoryId).length;
      const documentsWithTags = documents.filter(d => d.tags && d.tags.length > 0).length;
      const documentsWithExpiry = documents.filter(d => d.expiryDate).length;
      ...
      const processedDocuments = documents.filter(d => d.ocrProcessed).length;
      
      // Calculate individual scores (max 20 points each)
      const organizationScore = totalDocuments > 0 
        ? Math.min(20, Math.round((documentsWithCategories / totalDocuments) * 20))
        : 0;
      
      const completenessScore = totalDocuments > 0
        ? Math.min(20, Math.round(((documentsWithTags + documentsWithExpiry + processedDocuments) / (totalDocuments * 3)) * 20))
        : 0;
      
      const freshnessScore = totalDocuments > 0
        ? Math.min(20, Math.round((recentDocuments / Math.max(totalDocuments, 5)) * 20))
        : 0;
      
      // Sharing score (placeholder - would track document shares)
      const sharingScore = Math.min(5, Math.floor(totalDocuments / 10));
      
      // Security score (placeholder - would track secure practices)
      const securityScore = Math.min(20, 
        (documentsWithExpiry > 0 ? 5 : 0) + 
        (categories.length > 3 ? 5 : 0) +
        (processedDocuments > 0 ? 5 : 0) +
        (totalDocuments > 10 ? 5 : 0)
      );
      
      const totalScore = organizationScore + completenessScore + freshnessScore + sharingScore + securityScore;
      const maxScore = 100;
      ...
      const levelProgress = nextThreshold > totalScore ? 
        ((totalScore - levelThresholds[currentLevel]) / (nextThreshold - levelThresholds[currentLevel])) * 100 : 100;
      ...
      const achievements = [
        {
          ...
          unlocked: totalDocuments > 0,
          progress: totalDocuments > 0 ? 100 : 0,
          icon: "file"
        },
        {
          id: "organized",
          name: "Getting Organized",
          description: "Categorize 10 documents",
          unlocked: documentsWithCategories >= 10,
          progress: Math.min(100, (documentsWithCategories / 10) * 100),
          icon: "target"
        },
        {
          id: "tag_master",
          name: "Tag Master",
          description: "Add tags to 5 documents",
          unlocked: documentsWithTags >= 5,
          progress: Math.min(100, (documentsWithTags / 5) * 100),
          icon: "tags"
        },
        {
          id: "security_conscious",
          name: "Security Conscious",
          description: "Set expiry dates on important documents",
          unlocked: documentsWithExpiry >= 3,
          progress: Math.min(100, (documentsWithExpiry / 3) * 100),
          icon: "shield"
        },
        {
          id: "power_user",
          ...
          unlocked: totalDocuments >= 50,
          progress: Math.min(100, (totalDocuments / 50) * 100),
          icon: "zap"
        }
      ];
      ...
      const tips = [];
      if (organizationScore < 15) {
        tips.push("Try categorizing your documents to boost your organization score!");
      }
      if (documentsWithTags < totalDocuments * 0.3) {
        tips.push("Adding tags makes documents easier to find and increases completeness.");
      }
      if (recentDocuments === 0 && totalDocuments > 0) {
        tips.push("Keep your documents current by uploading recent files.");
      }
      if (documentsWithExpiry < 3) {
        tips.push("Set expiry dates on important documents like contracts and certificates.");
      }
      if (processedDocuments < totalDocuments * 0.5) {
        tips.push("Enable OCR processing to make your documents searchable.");
      }
